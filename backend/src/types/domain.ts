...
export const ACTIVITY_TYPES = [
  ...
  // These are GENERATED BY the Reward Engine (never rewarded themselves), so the
  // engine ignores them — this is what keeps the event bus from recursing.
  'xp-awarded',
  'level-up',
  'streak-increased',
  'streak-broken',
...
/**
 * The activity types that EARN a reward. The Reward Engine only acts on these;
 * every other activity type (including the gamification events above) is a
 * no-op, which is precisely what prevents the event bus from looping. Actual XP
 * values live in `config/gamification.ts`, never hardcoded in the engine.
 */
export const REWARDABLE_ACTIVITY_TYPES = [
  'problem-solved',
  'topic-completed',
  'phase-completed',
  'revision-completed',
...
/**
 * The kind of reward a RewardHistory row represents. Sprint 1 only mints XP;
 * the enum is the extension seam for Sprint 2 (badges, achievements) — no schema
 * change needed then, just a new member and a new minting path.
 */
export const REWARD_TYPES = ['xp', 'badge', 'achievement'] as const;
export type RewardType = (typeof REWARD_TYPES)[number];

/** Coarse module a reward originated from — used for grouping/filtering. */
export const REWARD_SOURCE_MODULES = ['learning', 'revision', 'knowledge', 'contest'] as const;
export type RewardSourceModule = (typeof REWARD_SOURCE_MODULES)[number];
